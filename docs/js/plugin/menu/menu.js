    ...
    function loadPlugin() {
      ...
      if (initialise) {
        ...
        function reenableMouseSelection() {
          ...
          select("nav.slide-menu").addEventListener("mousemove", function fn(
            e
          ) {
          ...
          if (isOpen()) {
            ...
            switch (event.keyCode) {
              ...
              case 68:
                ...
                var lastVisible =
                  itemsBelow.length > 0 &&
                  Math.abs(visibleOffset(itemsBelow[0])) <
                    itemsBelow[0].clientHeight
                    ? itemsBelow[0]
        ...
        function openMenu(event) {
          ...
          if (!isOpen()) {
            ...
            if (themes) {
              ...
              selectAll(
                'li[data-theme="' +
                  select("link#theme").getAttribute("href") +
                  '"]'
              ).forEach(function(i) {
            ...
            if (transitions) {
              selectAll('div[data-panel="Transitions"] li').forEach(function(
                i
              ) {
                i.classList.remove("active");
              });
              selectAll(
                'li[data-transition="' + Reveal.getConfig().transition + '"]'
              ).forEach(function(i) {
                i.classList.add("active");
              });
            }
        ...
        function openPanel(event, ref) {
          ...
          select(
            ".slide-menu-toolbar > li.active-toolbar-button"
          ).classList.remove("active-toolbar-button");
        ...
        function nextPanel() {
          var next =
            (parseInt(
              select(".active-toolbar-button").getAttribute("data-button")
            ) +
              1) %
            buttons;
          openPanel(
            null,
            select(
              '.toolbar-panel-button[data-button="' + next + '"]'
        ...
        function init() {
          if (!initialised) {
            ...
            if (typeof width === "string") {
              if (
                ["normal", "wide", "third", "half", "full"].indexOf(width) != -1
              ) {
